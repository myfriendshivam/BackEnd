...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const options = {
        httpOnly: true,
        secure: false // in localhost secure is false but in production it is true
    }
...
const logoutUser = asyncHandler(async (req, res) => {
    ...
    await User.findByIdAndUpdate(
        ...
        {
            $unset: {
                refreshToken: 1 // this is removes the field from document
            }
        },
        {
            new: true
...
const refreshAccessToken = asyncHandler(async (req, res) => {
    // if any one hit the endpoint then using cookies we access the refresh token
    ...
    try {
        ...
        // take refresh token id in DB
        const user = await User.findById(decodedToken?._id)
        if (!user) {
            throw new ApiError(401, "Invalid Refresh Token")
        }

        // match incoming token with user db token
...
const updateUserAvatar = asyncHandler( async(req, res) => {
    ...
    // TODO: delete old image - assignment (utility)
    const avatar = await uploadOnCloudinary(avatarLocalPath)
    if(!avatar.url){
